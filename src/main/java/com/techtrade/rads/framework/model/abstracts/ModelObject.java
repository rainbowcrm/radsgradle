...
public abstract class ModelObject {
			...
			while (itkeys.hasNext()) {
				...
				if (ModelObject.class.isAssignableFrom(curMethod.getParameters()[0].getType())){
					...
					if( !Utils.isNullString(valueStr) &&	!Utils.isNullString(valueStr)) {
						Object paramValue   = Utils.stringToType( valueStr, curMethod.getParameters()[0].getType(),context);
		...
		try {
			...
			while (it.hasNext()) {
				...
				if(ModelObject.class.isAssignableFrom(curMethod.getParameters()[0].getType())) {
					...
					Object paramValue   = Utils.stringToType( String.valueOf(value).trim(), curMethod.getParameters()[0].getType(),context);
		...
		for (XMLElement childElement : rootElement.getAllChildElements()) {
			...
			if (ModelObject.class.isAssignableFrom(curMethod.getParameters()[0].getType())){
				...
				if(!Utils.isNullString(childElement.getValue()) && !Utils.isNullString(childElement.getValue().trim())) {
					Object paramValue   = Utils.stringToType( childElement.getValue().trim(), curMethod.getParameters()[0].getType(),context);
					curMethod.invoke(object, paramValue);
				}
			}
		}
	...
	public String toBusinessKeyXML()  {
		return getXML("BK");
	}
	
	public String toPrimaryKeyXML()  {
		return getXML("PK");
	}
	 ...
	 * @return
	 */
	private String getXML(String mode) {
				...
				continue;
				 ...
				 if (ret == null) 
					 ...
					 if (radsProp!=null && radsProp.usePKForXML() ==true )
						 xmlString.append(((ModelObject)ret).toPrimaryKeyXML());
					 else if (radsProp!=null && radsProp.useBKForXML() ==true )
						 xmlString.append(((ModelObject)ret).toBusinessKeyXML());
					 else
						 xmlString.append(((ModelObject)ret).toXML());
					 ...
					 while (it.hasNext()) {
						 ...
						 if (itObject instanceof ModelObject) {
							 xmlString.append("<" + xmlTag + ">\n");
							 if (radsProp!=null && radsProp.usePKForXML() ==true )
								 xmlString.append(((ModelObject)itObject).toPrimaryKeyXML());
							 else if (radsProp!=null && radsProp.useBKForXML() ==true )
								 xmlString.append(((ModelObject)itObject).toBusinessKeyXML());
							 else
								 xmlString.append(((ModelObject)itObject).toXML());
							 xmlString.append("</" + xmlTag + ">\n");
						 } else {
